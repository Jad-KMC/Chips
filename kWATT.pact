; @DEV
; This is the kWATT token contract. The functions here are mostly just basic coin tracking
; Nearly all of the functionality of this contract is through the hashrate rental contract

; SECTION 1: Minting and Burning of tokens tied to energy data inputs
; MINT and BURN capabilities allow for manual burning and minting of kWATT tokens via the signatory,
;     Or they also allow the Chips rental contract to burn kWATT tokens at the end of a rental.
;     The MINT capability is utilized by the Chips contract as well, for when a user wants to start a rental
;     Finally, the manual MINT functionality is for funding rewards for liquidity pools

; SECTION 2: Integration to energy-based use cases, and framework for configurable tokens like BTC/DOGE/KDA
; Aside from the (read-all) function, this functionality encompassed in the hashrate-rental contract.
;     The kWATT token contract simply enables the hashrate rental contract by existing

; SECTION 3: Stock split
; Most of the work for a stock split will occur offline. First, a snapshot needs to occur using `split-snapshot`
; After that, every user will be airdropped a specific amount of tokens based on how much the price is going down
; The price of the tokens will go down across all exchanges over time, and will update immediately on the Chips website
; Any price increases will strictly be eaten by the company. All existing kWATT tokens will increase in value simply
;       by the nature of the Chips.finance website increasing the price of kWATT tokens.

(module kWATT GOVERNANCE

  @doc "kWATT token smart contract"
  ...
  (defcap MINT:bool
    ( receiver:string
      ...
      (enforce-one "The only way to mint new kWATT tokens is through the chips contract on chain 1"
          [(enforce-chips)
          (enforce-guard (at 'guard (read token-table ROOT_ACCOUNT_ID ['guard])))])
  ...
  (defcap BURN:bool
    ...
    (enforce-one "The only way to burn kWATT tokens is through the chips contract on chain 1"
        [(enforce-chips)
        (enforce-guard (at 'guard (read token-table ROOT_ACCOUNT_ID ['guard])))])
  ...
  (defconst ROOT_ACCOUNT_ID:string "chips-signatory"
      " ID for the account which initially owns all the tokens. ")

  (defconst INITIAL_SUPPLY:decimal 250000.0
    " Initial supply of 9 million tokens. ")
  ...
  (defun split-snapshot ()
    @doc "Allows the contract administrator to snapshot all balances for use in the scenario of a kWATT `stock split`"
    ; This is used in the event that energy prices go down enough for the business to justify lowering external pricing
  ...
  (defun initialize:string ()
    ...
    (with-capability (GOVERNANCE)
      (with-capability (CREDIT "chips-signatory")
      (create-account "chips-signatory" (read-keyset 'chips-signatory))
      (if (= (at 'chain-id (chain-data)) MINT_CHAIN)
        (credit "chips-signatory" (read-keyset 'chips-signatory) INITIAL_SUPPLY)
        "Created ROOT account without minting.")
        (update token-table "chips signatory" { "balance" : INITIAL_SUPPLY })
