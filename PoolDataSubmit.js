...
async function setMined(coin, mined, caller) {
    ...
    const signAccount = `k:${process.env.PUBLIC_KEY}`
    const signWithKeypair = createSignWithKeypair(KEY_PAIR)
    const transactionBuilder = Pact.modules[contract]['insert-coin-mined'](coin, mined, caller)

    const unsignedTransaction = Pact.builder
        .execution(transactionBuilder)
        .setMeta({
            chainId: CHAIN_ID,
            senderAccount: signAccount,
            gasLimit: 10000
        })
        .addSigner(publicKey, (signFor) => [
            signFor('coin.GAS'),
            signFor(`${contract}.insert-coin-mined`),
            signFor(`${contract}.ACCOUNT_GUARD`, signAccount)
        ])
        .addData('account', signAccount)
        .setNetworkId(NETWORK_ID)
        .createTransaction()

    let signedTx
    try {
        signedTx = await signWithKeypair(unsignedTransaction)
    ...
    // Prepare the payload for preflight
